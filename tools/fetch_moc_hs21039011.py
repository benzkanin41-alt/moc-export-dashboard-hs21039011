...
def main() -> None:
    ...
    hs_lookup = request_json(
        session,
        "POST",
        f"{BASE_URL}/lookup/hscodelist{HS_CODE}/{HS_VERSION}",
        json_payload={"lang": "th"},
    )
    hs_name = hs_lookup.get("Name") or f"{HS_CODE} : {PRODUCT_NAME}"
    ...
    dataset = {
        "metadata": {
            ...
            "currency": "THB",
            "valueUnit": "บาท",
            "quantityUnit": "KGM ตาม MOC source",
        },
